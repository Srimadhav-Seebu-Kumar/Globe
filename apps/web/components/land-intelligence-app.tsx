...
import { FloatingLegend, type FloatingLegendHandle } from "./ui/FloatingLegend";
...
export const LandIntelligenceApp = () => {
  ...
  const legendRef = useRef<FloatingLegendHandle>(null);
  ...
  const handleViewportStats = useCallback((min: number, max: number) => {
    legendRef.current?.setRange(min, max);
  }, []);

  const handleSelectMarketGlobe = useCallback((id: string) => {
    setSelectedMarketId(id);
    setDrawerParcelId(null);
    setActiveView("markets");
  }, []);

  const handleSelectMarketExplore = useCallback((id: string) => {
    setSelectedMarketId(id);
    setDrawerParcelId(null);
  }, []);
  ...
  return (
    <>
      ...
      <AppShell
        ...
        legend={
          <FloatingLegend ref={legendRef} metric={activeMetric} />
        }
          ...
          <div className="relative h-full w-full" style={{ background: "var(--bg-void)" }}>
            ...
            >
              <GlobeCanvas
                ...
                onSelectMarket={handleSelectMarketGlobe}
              />
            </ErrorBoundary>
            {/* Vignette overlay */}
            <div
                  ...
                  <div className="flex-1 relative min-w-0" style={{ background: "var(--bg-void)" }}>
                    ...
                    >
                      <GlobeCanvas
                        ...
                        metric={activeMetric}
                        onViewportStats={handleViewportStats}
                        onSelectMarket={handleSelectMarketExplore}
                      />
                    </ErrorBoundary>

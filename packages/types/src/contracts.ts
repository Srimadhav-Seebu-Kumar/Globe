// Shared DTO contracts used by both `@globe/api` and `@globe/web`.
// Single source of truth — `apps/api/src/contracts.ts` re-exports from here and
// `apps/web/components/land-intelligence-app.tsx` imports from here so the two
// can never drift.
